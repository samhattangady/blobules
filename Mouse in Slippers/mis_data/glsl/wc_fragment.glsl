...
void main() {
    // fragColor = texture(spritesheet, textCoord.xy);
    // return;
    vec2 tx = textCoord.xy + 0.0004*cnoise(fragCoord.xy*15.0)*sin(time/3.5);
    tx = tx + 0.008*cnoise(fragCoord.xy*2.4)*sin(time/1.8);
    tx = textCoord.xy;
    vec4 col = texture(spritesheet, tx);
    vec4 shadow = texture(sdfsheet, vec2(tx.x, tx.y-tx.y*Y_DISPLACEMENT));
    ...
    col = mix(col, vec4(0.0, 0.0, 0.0, 1.0), shadow_dist*(0.3+0.1*sin(time/4.0))); 
    if (opacity > 0.01)
    ...
    if (fill_dist > 0.00) {
        ...
        bleed = smoothstep(0.00, 0.499, bleed);
        bleed *= bleed*0.5;
        bleed -= 0.5;
    ...
    line = smoothstep(0.012, 0.0, line) * abs(cnoise(fragCoord.xy*01.0*sin(time/4.2)));
    if (col.a > 0.0) {
        col = mix(vec4(0.94, 0.94, 0.92, 1.0), col, col.a);
        col.a = 1.0;
    }
